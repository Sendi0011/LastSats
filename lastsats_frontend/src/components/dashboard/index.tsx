    ...
    return (
      ...
      >
        <div style={{ textAlign: 'center', maxWidth: 400 }}>
          ...
          <Link h13px 28px', fontSize: 14, display: 'inline-block', textDecoration: 'none' }}>
            Go to Home
          </Link>
        </div>
      </div>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingTop: 64 }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 36 }}>
          ...
          <StatCard icon={<Zap size={alance" value={loadingBalances ? '···' : `${stxBalance.toFixed(0)} STX`} sub="For gas fees" accent="var(--accent-blue)" />
        ...
        {vaults.length === 0 ? (
          <div className="card" style={{ padding: '64px 32px', textAlign: 'center' }}>
            <Shielor="var(--text-muted)" style={{ margin: '0 auto 16px' }} />
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'var(--bg-primary)' }} className="grid-bg">
        <div style={{ textAlign: 'center', maxWidth: 400 }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 24, color: 'var(--text-primary)', marginBottom: 12 }}>Connect your wallet</h2>
          <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 28, lineHeight: 1.6 }}>Connect your Xverse or Leather wallet to access your LastSats dashboard.</p>
          <Link href="/" className="btn-primary" style={{ padding: '13px 28px', fontSize: 14, display: 'inline-block', textDecoration: 'none' }}>Go to Home</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingTop: 64 }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 36, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 30, color: 'var(--text-primary)', letterSpacing: '-0.025em', marginBottom: 6 }}>My Vaults</h1>
            <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
              {vaults.length} vault{vaults.length !== 1 ? 's' : ''} protecting{' '}
              <span style={{ color: 'var(--accent-orange)', fontWeight: 600 }}>{totalProtected.toFixed(5)} sBTC</span>
            </p>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={refreshBalances} disabled={loadingBalances} title="Refresh on-chain balances"
              style={{ padding: '11px 14px', borderRadius: 10, background: 'var(--bg-card)', border: '1px solid var(--border-bright)', color: loadingBalances ? 'var(--text-muted)' : 'var(--text-secondary)', cursor: loadingBalances ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', fontSize: 14, transition: 'all 0.2s' }}>
              <TrendingUp size={15} style={{ animation: loadingBalances ? 'spin 1s linear infinite' : 'none' }} />
            </button>
            <button onClick={() => setShowCreate(true)} className="btn-primary" style={{ padding: '11px 22px', fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Plus size={16} /> New Vault
            </button>
          </div>
        </div>
        ...
        {vaults.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16 }}>
            ...
            <div onClick={() => setShowCreate(true)} className="card"
              style={{ padding: 24, cursor: 'pointer', border: '1px dashed var(--border-bright)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, minHeight: 200, transition: 'all 0.2s' }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(249,115,22,0.4)'; e.currentTarget.style.background = 'rgba(249,115,22,0.04)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border-bright)'; e.currentTarget.style.background = 'var(--bg-card)'; }}>
              <div style={{ width: 44, height: 44, borderRadius: 11, border: '1px dashed rgba(249,115,22,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-orange)', opacity: 0.7 }}>
